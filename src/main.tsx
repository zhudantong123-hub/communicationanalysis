...
function MultiObjectiveContribution(){
 ...
 return <><PublisherDistribution/><section className="multiObjective"><div><h3>Feed 多目标贡献</h3><p>推荐页流量按队列类型拆分，以列表展示各目标对 VV 的贡献。</p></div><div className="objectiveTabs">{Object.keys(groups).map(item=><button key={item} className={group===item?'selected':''} onClick={()=>setGroup(item)}>{item} <span>{groups[item].share}</span></button>)}</div><div className="objectiveList objectiveListOnly"><header><span>排名</span><b>多目标分析项</b><em>贡献 VV</em><strong>贡献占比</strong></header>{current.rows.map((row,index)=><div key={row[0]}><i>{index+1}</i><span>{row[0]}</span><em>{row[1]}</em><strong>{row[2]}</strong></div>)}</div></section></>
}
