...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 const [knowledgeTag,setKnowledgeTag]=useState('系统推荐')
 ...
 const viewConfig:any={
  传播承接:{question:'内容从哪里来，经过谁，传到哪里？',decision:'优先切断“搜索回流 → 搬运账号簇”路径，可减少约 31% 后续风险曝光。',tags:['系统推荐','首发','推荐放大','搜索回流','分享扩散','跨圈','长尾']},
 ...
 const toggleType=(type:string)=>{setDrilledId(null);setNodeTypes(current=>type==='全部'?['全部']:current.includes(type)?(current.filter(item=>item!==type).length?current.filter(item=>item!==type):['全部']):[...current.filter(item=>item!=='全部'),type])}
 const rootCandidates=nodes.filter(node=>node.views.includes(view)&&node.arrival<=timeIndex&&(nodeTypes.includes('全部')||nodeTypes.includes(node.kind)))
 const isRecommended=knowledgeTag==='系统推荐'
 const matchedCandidates=isRecommended?rootCandidates:rootCandidates.filter(node=>node.tags.includes(knowledgeTag))
 const candidateIds=new Set(rootCandidates.map(node=>node.id))
 const candidateEdges=edges.filter(edge=>edge.views.includes(view)&&candidateIds.has(edge.from)&&candidateIds.has(edge.to))
 const matchedIds=new Set(matchedCandidates.map(node=>node.id))
 const relatedIds=new Set<string>()
 if(!isRecommended)candidateEdges.forEach(edge=>{if(matchedIds.has(edge.from)&&!matchedIds.has(edge.to))relatedIds.add(edge.to);if(matchedIds.has(edge.to)&&!matchedIds.has(edge.from))relatedIds.add(edge.from)})
 const relatedCandidates=isRecommended?[]:rootCandidates.filter(node=>relatedIds.has(node.id))
 const contextCandidates=isRecommended?[]:rootCandidates.filter(node=>!matchedIds.has(node.id)&&!relatedIds.has(node.id))
 const focusPositions=[[430,135],[610,135],[520,250],[420,385],[620,385],[520,450]]
 const relatedPositions=[[815,125],[975,175],[875,280],[980,400],[800,410],[905,455]]
 const contextPositions=[[95,145],[235,120],[170,270],[90,400],[245,410],[165,465]]
 const rootVisibleNodes=isRecommended?rootCandidates:rootCandidates.map(node=>{
  const matchIndex=matchedCandidates.findIndex(item=>item.id===node.id)
  const relatedIndex=relatedCandidates.findIndex(item=>item.id===node.id)
  const contextIndex=contextCandidates.findIndex(item=>item.id===node.id)
  const position=matchIndex>=0?focusPositions[matchIndex%focusPositions.length]:relatedIndex>=0?relatedPositions[relatedIndex%relatedPositions.length]:contextPositions[contextIndex%contextPositions.length]
  return {...node,x:position[0],y:position[1],clusterGroup:matchIndex>=0?'primary':relatedIndex>=0?'related':'context'}
 })
 const rootVisibleIds=new Set(rootVisibleNodes.map(node=>node.id))
 const rootVisibleEdges=candidateEdges.filter(edge=>rootVisibleIds.has(edge.from)&&rootVisibleIds.has(edge.to))
 ...
 const chooseView=(next:RelationView)=>{setView(next);setDrilledId(null);setKnowledgeTag('系统推荐');setNodeTypes(['全部']);setSelectedId(next==='设备聚合'?'deviceD07':next==='群聊聚合'?'groupA':'recommend')}
 ...
 const applyTag=(tag:string)=>{setDrilledId(null);setKnowledgeTag(tag);const firstMatch=nodes.find(node=>node.views.includes(view)&&node.tags.includes(tag));setSelectedId(tag==='系统推荐'?(view==='设备聚合'?'deviceD07':view==='群聊聚合'?'groupA':'recommend'):(firstMatch?.id||selectedId))}
 const current=viewConfig[view]
 const clusterSummary=isRecommended?current.decision:`已按“${knowledgeTag}”重新聚类：形成 1 个核心特征簇、${relatedCandidates.length?'1 个关联传播簇':'0 个关联传播簇'}和${contextCandidates.length?'1 个其他特征簇':'0 个其他特征簇'}。`
 const displayNodeMap=new Map(displayNodes.map((node:any)=>[node.id,node]))
 const clusterEdgeIsFocused=(edge:any)=>isRecommended||edge.tag===knowledgeTag||displayNodeMap.get(edge.from)?.clusterGroup==='primary'||displayNodeMap.get(edge.to)?.clusterGroup==='primary'
 const clusterGroups=!isRecommended?[{id:'context',title:'其他特征簇',count:contextCandidates.length,detail:'保留为分析上下文'},{id:'primary',title:`“${knowledgeTag}”核心簇`,count:matchedCandidates.length,detail:'Tag 特征直接命中'},{id:'related',title:'关联传播簇',count:relatedCandidates.length,detail:'由一跳关系自动归入'}].filter(group=>group.count>0):[]
 const stageLabels=!isRecommended?['其他特征簇',`“${knowledgeTag}”核心簇`,'关联传播簇']:view==='设备聚合'?['设备中心','关联账号','异常行为','扩散结果']:view==='群聊聚合'?['内容输入','核心群聊','桥接用户','站内回流']:['传播源头','首次放大','异常承接','长尾扩散']
 return <div className="relationDecisionExplorer">
  <div className="relationDecisionHead"><div><b>传播关系链 <SourceBadge type="ai"/></b><span>{current.question}</span></div><div className="relationDecisionSummary"><Sparkles size={14}/><span><small>{isRecommended?'系统推荐聚类':'聚类结果'}</small><b>{clusterSummary}</b></span></div></div>
  ...
  <div className="relationTags"><div><span>节点类型</span>{typeOptions.map(item=><button key={item} className={nodeTypes.includes(item)?'selected':''} onClick={()=>toggleType(item)}>{item}</button>)}</div><div><span>聚类依据</span>{current.tags.map((item:string)=><button key={item} className={knowledgeTag===item?'selected':''} onClick={()=>applyTag(item)} title={item==='系统推荐'?'恢复系统综合聚类':`以“${item}”为主要特征重新聚类，不隐藏其他节点`}>{item}</button>)}</div></div>
  <div className="relationBreadcrumb"><button onClick={()=>{setDrilledId(null);setKnowledgeTag('系统推荐')}}>事件全景</button><ChevronRight size={12}/><button onClick={()=>setDrilledId(null)}>{view}</button>{!isRecommended&&!currentDrill&&<><ChevronRight size={12}/><b>按“{knowledgeTag}”重聚类</b></>}{currentDrill&&<><ChevronRight size={12}/><b>{currentDrill.title}</b><button className="relationBack" onClick={()=>setDrilledId(null)}>返回上一级</button></>}<em>{isRecommended?'系统综合聚类':`核心簇 ${matchedCandidates.length} 个节点`} · 全图保留 {displayNodes.length} 个节点</em></div>
  <div className="relationWorkspace">
   <div className={'relationMap'+(currentDrill?' hasExpansion':'')+(!isRecommended?' reclustered':'')}><div className="relationStageLabels">{stageLabels.map((item:string)=><span key={item}>{item}</span>)}</div><div className="relationMapScene" style={{transform:`scale(${zoom})`}}>{clusterGroups.map((group:any)=><div key={group.id} className={'relationAutoCluster '+group.id}><span><b>{group.title}</b><small>{group.count} 个节点 · {group.detail}</small></span></div>)}<svg viewBox="0 0 1100 520" preserveAspectRatio="none">{displayEdges.map((edge:any)=>{const a=points[edge.from],b=points[edge.to];const focused=clusterEdgeIsFocused(edge);return <g key={edge.from+edge.to} className={focused?'clusterFocused':'clusterContext'}><path className={edge.tone+(selectedId===edge.from||selectedId===edge.to?' active':'')} d={`M${a[0]} ${a[1]} C${(a[0]+b[0])/2} ${a[1]},${(a[0]+b[0])/2} ${b[1]},${b[0]} ${b[1]}`}/><text x={(a[0]+b[0])/2} y={(a[1]+b[1])/2-5}>{edge.tag}</text></g>})}</svg>{displayNodes.map((node:any)=><button key={node.id} className={'relationMapNode '+node.kind+' '+node.tone+(selectedId===node.id?' selected':'')+(node.drillKey?' drillable':'')+(!isRecommended?' cluster-'+node.clusterGroup:'')} style={{left:`${node.x/11}%`,top:`${node.y/5.2}%`}} onClick={()=>openNode(node)}><i>{node.kind==='视频'?<Play size={14}/>:node.kind==='账号'?<Users size={14}/>:node.kind==='设备'?<ScanLine size={14}/>:node.kind==='群聊'?<MessageCircle size={14}/>:node.kind==='渠道'?<Zap size={14}/>:<Network size={14}/>}</i><span><b>{node.title}</b><small>{node.meta}</small><em>{node.role}{node.drillKey?' · 可展开':''}</em></span></button>)}</div>{currentDrill&&<div className="relationClusterExpansion"><header><span><small>簇内展开 · {view}</small><b>{currentDrill.title}</b></span><button onClick={()=>setDrilledId(null)}><X size={13}/>收起</button></header><div className="relationExpansionMembers">{currentDrill.nodes.map((node:any)=><button key={node.id} className={node.tone+(selectedId===node.id?' selected':'')} onClick={()=>openNode(node)}><i>{node.kind==='视频'?<Play size={13}/>:node.kind==='账号'?<Users size={13}/>:node.kind==='设备'?<ScanLine size={13}/>:node.kind==='群聊'?<MessageCircle size={13}/>:<Zap size={13}/>}</i><span><small>{node.kind} · {node.role}</small><b>{node.title}</b><em>{node.meta}</em></span>{node.drillKey?<ChevronRight size={13}/>:node.item?<FileText size={12}/>:null}</button>)}</div><footer><span>簇内 {currentDrill.nodes.length} 个节点 · {currentDrill.edges.length} 条关系</span><b>点击实体查看证据；点击跨簇出口继续穿透</b></footer></div>}<div className="relationMapLegend"><span><i/>核心聚类关系</span><span><i/>跨簇/上下文关系</span><small>{currentDrill?'已在原图内展开成员，背景保留全局位置与上下游关系':isRecommended?'系统综合多类关系自动聚类':`已按“${knowledgeTag}”生成多个关系簇；点击簇内节点继续展开`}</small></div></div>
   <aside className="relationInspector"><div className="relationMiniMap"><span>{currentDrill?'簇内位置':'全景位置'}</span><i className={'p '+selected.tone} style={{left:(selected.x/1100*100)+'%',top:(selected.y/520*100)+'%'}}/></div><header><small>{selected.kind} · {selected.role}</small><b>{selected.title}</b><span>{selected.meta}</span></header><dl><div><dt>聚类解释</dt><dd>{selected.tags.slice(0,3).join(' · ')}</dd></div><div><dt>{currentDrill?'簇内关系':'全局关系'}</dt><dd>输入 {selectedEdges.filter((edge:any)=>edge.to===selected.id).length} 条 · 输出 {selectedEdges.filter((edge:any)=>edge.from===selected.id).length} 条</dd></div><div><dt>风险状态</dt><dd>{selected.tone==='risk'?'高风险，建议优先核查':selected.tone==='warning'?'传播异常，需持续观察':'正常承接关系'}</dd></div></dl><section><b>{currentDrill?'跨簇关系摘要':'自动聚类说明'}</b><p>{currentDrill?'成员在原图内展开，全局节点和关系仍保留在背景；跨簇出口可以继续进入相邻聚合簇。':'系统综合关系强度、内容特征和传播行为形成当前聚类；选择聚类依据只会重排和强化关系，不会隐藏其他节点。'}</p></section>{selected.drillKey?<button onClick={()=>drillInto(selected)}>展开“{selected.title}”成员网络</button>:selected.item&&selected.kind==='视频'?<button onClick={()=>select('video',selected.item)}>查看视频详情与证据</button>:selected.item&&selected.kind==='账号'?<button onClick={()=>select('account',selected.item)}>查看账号详情与证据</button>:<button onClick={()=>applyTag(selected.tags[0])}>以“{selected.tags[0]}”重新聚类</button>}</aside>
  </div>
 </div>
}
