...
function VideoChain({select}){
 ...
 const pos=chainVideos.map((_,index)=>{
  if(!activeCluster){
   if(index===0)return [525,290]
   const outer=index>8,slot=outer?index-9:index-1,total=outer?15:8,radius=outer?430:255,angle=-Math.PI/2+slot/total*Math.PI*2
   return [600-75+Math.cos(angle)*radius,360-70+Math.sin(angle)*radius*.56]
  }
