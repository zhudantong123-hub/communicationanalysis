...
function App(){
 ...
 return <div className="app">
  ...
  <div className={'layout '+(detailOpen?'':'full')}><section className="left">
   ...
   {artifactOpen?<>
   <div className="hero"><h1><span className="eventGrade" aria-label="指令等级 S，舆情等级 A"><i>S</i><em>|</em><b>A</b></span>“{reportEvent}”舆情传播分析</h1><p>{reportEvent==='揭阳虐狗事件'?'2026 年 2 月 5 日，广东揭阳一段疑似虐狗视频在多个平台传播，引发对涉事人员责任、动物保护立法、视频真实性及网络暴力边界的集中讨论。相关部门已介入核查，部分网传细节仍待权威确认。':`AI 已围绕“${reportEvent}”完成内容聚合、传播趋势识别、观点聚类和账号关系分析。当前报告展示模拟的全链路研判结果，可继续通过左侧对话追加分析要求。`}</p><footer><span><Clock3/>传播周期：02-05 15:45 至 02-12 18:00</span><span><FileText/>事件摘要：已完成传播态势、内容观点与关键账号综合研判</span><span className="sourceLegend"><i className="dataSource"/>数据统计 <i className="aiSource"/>AI 生成 / 研判</span></footer></div>
   <InsightCommandCenter select={select}/>
...
function InsightCommandCenter({select}){
 ...
 const peakNodes=[{x:155,y:83,peak:peaks[0]},{x:305,y:38,peak:peaks[1]},{x:455,y:66,peak:peaks[2]}]
 return <section className="commandCenter"><div className="commandHead"><div><h2>传播态势与异常下钻</h2><p>从异常峰值定位驱动内容、关键账号与建议处置动作。</p></div></div><div className="trendGrid"><div className="trendPanel"><div className="panelTitle"><b>传播趋势</b><span>视频量（条） / VV（万） / 舆情声量（万）</span></div><div className="trendLegend"><span className="video"><i/>视频量（条）</span><span className="vv"><i/>VV（万）</span><span className="opinion"><i/>舆情声量（万）</span><span className="signal command"><i/>指令</span><span className="signal public"><i/>舆情</span><span className="signal ai"><i/>AI 分析</span></div><div className="trendChart"><svg viewBox="0 0 620 155" preserveAspectRatio="none"><path className="area vvArea" d="M0 135 L55 126 L105 119 L155 83 L205 108 L255 93 L305 38 L355 79 L405 91 L455 66 L505 98 L560 107 L620 121 L620 155 L0 155Z"/><path className="line videoLine" d="M0 126 L55 119 L105 113 L155 97 L205 104 L255 82 L305 62 L355 75 L405 84 L455 73 L505 89 L560 98 L620 110"/><path className="line vvLine" d="M0 135 L55 126 L105 119 L155 83 L205 108 L255 93 L305 38 L355 79 L405 91 L455 66 L505 98 L560 107 L620 121"/><path className="line opinionLine" d="M0 142 L55 137 L105 128 L155 107 L205 115 L255 102 L305 74 L355 91 L405 82 L455 88 L505 104 L560 115 L620 125"/><line x1="155" y1="20" x2="155" y2="145"/><line x1="305" y1="20" x2="305" y2="145"/><line x1="455" y1="20" x2="455" y2="145"/></svg>{peakNodes.map(({x,y,peak})=><button key={peak.time} className={'peakMarker signalMarker '+peak.type+(activePeak===peak.time?' active':'')} style={{left:(x/620*100)+'%',top:(y/155*112)+'px'}} aria-label={peak.title} onMouseEnter={()=>setActivePeak(peak.time)} onMouseLeave={()=>setActivePeak(null)} onFocus={()=>setActivePeak(peak.time)} onBlur={()=>setActivePeak(null)} onClick={()=>{setActivePeak(activePeak===peak.time?null:peak.time);select('path',{name:peak.title})}}><span><i/>{peak.typeName} · {peak.title}</span></button>)}<div className="trendLabels"><span>02-05</span><span>02-06</span><span>02-08</span><span>02-12</span></div></div></div><div className="anomalyPanel"><div className="panelTitle"><b>异常信号 <SourceBadge type="ai"/></b><span className="riskDot">3 项待研判</span></div>{peaks.map(peak=><button key={peak.time} className={activePeak===peak.time?'active':''} onMouseEnter={()=>setActivePeak(peak.time)} onMouseLeave={()=>setActivePeak(null)} onFocus={()=>setActivePeak(peak.time)} onBlur={()=>setActivePeak(null)} onClick={()=>{setActivePeak(activePeak===peak.time?null:peak.time);select('path',{name:peak.title})}}><i className={'signalIcon '+peak.type}><TriangleAlert size={13}/></i><span><b>{peak.title}<small className={'signalType '+peak.type}>{peak.typeName}</small></b><small>{peak.time} · {peak.desc}</small></span><em>{peak.growth}</em></button>)}</div></div></section>
}
const trafficTree=[
...
function InteractiveSankey({select,onVideoFilter,activeVideoFilter,activeOpinion}:{select:(type:string,item:any)=>void,onVideoFilter:(keyword:string|null)=>void,activeVideoFilter:string|null,activeOpinion:string|null}){
 ...
 useEffect(()=>{if(!activeOpinion)return;setSourceId('feed');setFeedLayerId('m1');onVideoFilter(null)},[activeOpinion])
 useEffect(()=>{
  const canvas=canvasRef.current
  if(!canvas)return
  const update=()=>setCanvasWidth(canvas.clientWidth||1000)
  update()
 ...
 const analysis=source.id==='feed'?{...analysisBase,interactions:thirdItems.map(item=>[item[0],item[1]+'万',(item[1]/source.value*100).toFixed(1)+'%'])}:analysisBase
 return <div className="hierSankey channelSankey"><div className="sankeyHead"><div><b>渠道传播路径</b><span>流量渠道 → 下钻分析层；流量单位：万</span></div><div className="pathCrumb"><span>{source.name}</span><ChevronRight/><b>{dimensionLabel}</b></div></div><div ref={canvasRef} className={'flowCanvas twoLevel '+(source.id==='feed'?'feedThreeLevel allFeedTargets':'')}><svg viewBox={`0 0 ${canvasWidth} 560`} preserveAspectRatio="none">{source.children.map((item,i)=><path key={'s-'+item.id} className="active" d={`M${firstRight} ${sourceY} C${firstRight+(secondLeft-firstRight)*.42} ${sourceY} ${firstRight+(secondLeft-firstRight)*.58} ${secondY[i]} ${secondLeft} ${secondY[i]}`} style={{strokeWidth:Math.max(7,item.value/source.value*42)}}/>)}{source.id==='feed'&&feedThirdItems.map(({parent,parentIndex,item,index},allIndex)=><path key={parent.id+'-'+item[0]} className="tertiary" d={`M${secondRight} ${secondY[parentIndex]} C${secondRight+(thirdLeft-secondRight)*.42} ${secondY[parentIndex]} ${secondRight+(thirdLeft-secondRight)*.58} ${56+allIndex*30} ${thirdLeft} ${56+allIndex*30}`} style={{strokeWidth:Math.max(2,item[1]/source.value*18)}}/>)}</svg><div className="flowColumn levelOne"><h4>一级 · 流量渠道（单选）</h4>{trafficTree.map(item=><button key={item.id} className={source.id===item.id?'selected':''} aria-pressed={source.id===item.id} onClick={()=>chooseSource(item)}><span>{item.name}<small>{item.share}</small></span><b>{item.value.toLocaleString()}万</b></button>)}</div><div className="flowColumn levelTwo"><h4>二级 · {dimensionLabel}</h4>{source.children.map(item=><button key={item.id}><span>{item.name}<small>{(item.value/source.value*100).toFixed(1)}%</small></span><b>{item.value.toLocaleString()}万</b></button>)}</div>{source.id==='feed'&&<div className="flowColumn levelThree actionColumn"><h4>三级 · 全部业务子类</h4>{feedThirdItems.map(({parent,item})=><button key={parent.id+'-'+item[0]} onClick={()=>onVideoFilter(item[0])}><span><i className={'feedGroupDot '+parent.id}/>{item[0]}<small>{(item[1]/parent.value*100).toFixed(2)}%</small></span><b>{item[1].toLocaleString()}万</b></button>)}</div>}</div>{source.id==='feed'&&<MultiObjectiveContribution/>}<div className="channelAnalysis"><div className="analysisTitle"><div><b>{analysis.title}</b><span>{analysis.dimension}</span></div><div><em>{source.name} 流量 {source.value.toLocaleString()}万</em><span>数据范围：近 7 天</span></div></div><div className="analysisTable"><div className="analysisRow head"><span>类型</span><b>分析项</b><em>{source.id==='search'?'搜索次数':'流量'}</em><i>渠道占比</i><u>关键指标</u></div>{analysis.rows.map((row,index)=><button className={'analysisRow '+(activeVideoFilter===row[1]?'selected':'')} aria-pressed={activeVideoFilter===row[1]} key={row[1]} onClick={()=>onVideoFilter(row[1])}><span>{row[0]}</span><b><small>{index+1}</small>{row[1]}</b><em>{row[2]}</em><i>{row[3]}</i><u>{row[4]}</u><ChevronRight/></button>)}</div><div className="analysisInsight"><b>自动洞察</b><span>点击任一分析层或分析项，即可联动筛选下方代表视频。</span></div></div><div className="sankeyFoot"><span>当前选中渠道</span><b>{source.name}</b><em>全站流量占比 {source.share}</em><i>点击分析层或分析项可筛选下方代表视频</i></div></div>
}
...
function MultiObjectiveContribution(){
 ...
 const groups:any={主队列:{share:'85.85%',rows:[['基础消费·时长','19.01万','17.62%'],['基础消费·关注','18.37万','17.03%'],['基础消费·社交','12.88万','11.94%'],['基础消费·投稿','12.57万','11.66%'],['基础消费·评论','10.20万','9.45%']]},UGC队列:{share:'13.01%',rows:[['内容创作·投稿','4.23万','4.12%'],['互动创作·评论','3.46万','3.37%'],['作者主页回访','2.88万','2.81%'],['同城内容消费','2.79万','2.71%']]},冷启动队列:{share:'1.14%',rows:[['新内容探索','1.28万','0.62%'],['新人作者扶持','0.64万','0.31%'],['兴趣扩展','0.43万','0.21%']]}}
 const current=groups[group]
 return <><PublisherDistribution/><section className="multiObjective"><div><h3>Feed 多目标贡献</h3><p>推荐页流量按队列类型拆分，展示各目标对 VV 的贡献。</p></div><div className="objectiveTabs">{Object.keys(groups).map(item=><button key={item} className={group===item?'selected':''} onClick={()=>setGroup(item)}>{item} <span>{groups[item].share}</span></button>)}</div><div className="objectiveBody"><div className="objectiveTrend"><b>{group}多目标贡献趋势</b><div className="objectiveBars">{[88,96,71,54,42,35,28,22,18,26,33,46].map((height,index)=><i key={index} style={{height:(height-(group==='UGC队列'?18:group==='冷启动队列'?38:0))+'%'}}/>)}</div><span>按小时聚合 · VV 贡献</span></div><div className="objectiveList"><b>{group}多目标贡献详情</b>{current.rows.map(row=><div key={row[0]}><span>{row[0]}</span><em>{row[1]}</em><strong>{row[2]}</strong></div>)}</div></div></section></>
}
...
function Stats({open,setOpen,select,addToChat}){
 ...
 const filteredVideos=(videoFilter||activeOpinion)?displayVideos.filter(video=>effectiveVideoIds.includes(video.id)):displayVideos
 return <><OpinionDistribution activeOpinion={activeOpinion} onSelect={setActiveOpinion}/><Section title="传播路径分布" desc={activeOpinion?`已按核心观点“${activeOpinion}”联动展示对应传播渠道`:'点击上方核心观点，联动查看对应传播路径'} action={activeOpinion?<button className="videoFilterChip" onClick={()=>setActiveOpinion(null)}>{activeOpinion}<X size={12}/></button>:null} source="data"/><InteractiveSankey select={select} activeVideoFilter={videoFilter} onVideoFilter={setVideoFilter} activeOpinion={activeOpinion}/>
 ...
 <div id="filtered-video-results" className={(videoFilter||activeOpinion)?'isFiltered':''}><Section title="热点观点与代表视频" desc={activeOpinion?`已按观点“${activeOpinion}”筛选，当前展示 ${filteredVideos.length} 条代表视频`:videoFilter?`已按分析项“${videoFilter}”筛选，共 ${filteredVideos.length} 条代表视频`:'基于统计数据汇总代表视频，AI 辅助完成文案、OCR 和 ASR 语义聚合'} action={(videoFilter||activeOpinion)?<button className="videoFilterChip" onClick={()=>{setVideoFilter(null);setActiveOpinion(null)}}><ScanLine size={12}/>{activeOpinion||videoFilter}<X size={12}/></button>:<button className="text">查看全部 26 个观点 <ChevronRight size={14}/></button>} source="data"/><div className="cards videoListGrid">{filteredVideos.map((v,i)=><VideoCard v={v} i={i} select={select} addToChat={addToChat}/>)}</div></div></>
}
...
function VideoDynamicLinks({pos,arrival,timeIndex,cluster}:{pos:number[][],arrival:number[],timeIndex:number,cluster:any}){
 ...
 const edges=cluster?expandedEdges:[[0,1],[0,2],[0,3],[0,4],[1,5],[2,6],[3,7],[3,8],[4,9],[1,7],[2,8],[4,6]]
 const allowed=(index:number)=>arrival[index]<=timeIndex&&(!cluster||cluster.indices.includes(index))
 return <svg className="links dynamicVideoLinks" viewBox="0 0 1200 720">{edges.filter(([from,to])=>allowed(from)&&allowed(to)).map(([from,to],index)=>{const x1=pos[from][0]+150,y1=pos[from][1]+72,x2=pos[to][0],y2=pos[to][1]+72;return <path key={from+'-'+to} className={index>4?'dash':''} d={`M${x1} ${y1} C${x1+70} ${y1} ${x2-70} ${y2} ${x2} ${y2}`}/>})}</svg>
}
...
function VideoChain({select}){
 ...
 const videoClusters=[
  {id:'position',name:'名不副实者身居高位',summary:'围绕能力与职位不匹配展开讽刺和质疑',indices:[0,1,2,3,4],plays:'531.1万',risk:'高争议',x:210,y:155,w:230,h:150,c:'violet'},
  {id:'metaphor',name:'“乌龟上杆”隐喻解读',summary:'解释俗语来源、寓意及网络语境变化',indices:[5,6,7,8,9,10,11],plays:'335.5万',risk:'语义解读',x:520,y:125,w:205,h:132,c:'blue'},
  {id:'target',name:'影射对象猜测',summary:'评论区对号入座并猜测具体影射对象',indices:[12,13,14,15,16],plays:'227.8万',risk:'风险聚集',x:690,y:390,w:245,h:166,c:'orange'},
  {id:'power',name:'权力来源与推举关系',summary:'讨论谁将其推上高位及背后的利益网络',indices:[17,18,19,20,21,22,23],plays:'173.5万',risk:'观点发散',x:350,y:470,w:215,h:140,c:'cyan'}
 ]
 ...
 const clusterOverviewPos=[
  [525,290],[430,210],[620,210],[430,355],[620,355],
  [90,100],[240,80],[330,170],[90,220],[230,230],[350,90],[350,260],
  [760,100],[900,80],[1030,150],[760,230],[920,230],
  [280,450],[430,420],[580,450],[730,420],[850,500],[380,560],[650,560]
 ]
