...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 const chooseView=(next:RelationView)=>{setView(next);setCustomApplied(false);setDrilledId(null);setSelectedId(next==='设备聚合'?'deviceD07':next==='群聊聚合'?'groupA':'video-0')}
 ...
 const applyCustomRule=()=>{const next:RelationView=ruleTarget==='设备'?'设备聚合':ruleTarget==='群聊'?'群聊聚合':'传播承接';setView(next);setCustomApplied(true);setDrilledId(null);setSelectedId(next==='设备聚合'?'deviceD07':next==='群聊聚合'?'groupA':'video-0');setCustomOpen(false)}
 ...
 const clusterSurfaces=surfaceSets[view].map((surface:any,index:number)=>({id:`cluster-surface-${index}`,title:surface[0],meta:surface[1],members:surface[2],x:surfaceLayout[index][0],y:surfaceLayout[index][1],w:surfaceLayout[index][2],h:surfaceLayout[index][3],tone:index===2||index===3?'risk':index%2?'blue':'violet'}))
 const dotPositions=[[27,58],[57,38],[72,70],[43,78]]
 const openSurfaceMember=(member:any)=>{setSelectedId(member.id);if(member.drillKey){drillInto(member);return}if(member.item)select('video',member.item)}
 ...
 return <div className="relationDecisionExplorer">
  ...
  <ChainTimeline value={timeIndex} onChange={value=>{setTimeIndex(value);setDrilledId(null);onTimeChange(value)}} total={surfaceEntityCount+1} label="关系形成时间"/>
  ...
  <div className="relationWorkspace">
   <div className={'relationMap radialClusterMap'+(currentDrill?' hasExpansion':'')}><div className="relationMapScene radialClusterScene" style={{transform:`scale(${zoom})`}}><svg className="radialClusterLinks" viewBox="0 0 1100 520" preserveAspectRatio="none">{clusterSurfaces.map((surface:any,index:number)=>{const x=surface.x+surface.w/2,y=surface.y+surface.h/2;return <path key={surface.id} className={surface.tone==='risk'?'risk':index%2?'warning':''} d={`M550 260 C${550+(x-550)*.35} ${260+(y-260)*.08},${550+(x-550)*.7} ${y-(y-260)*.08},${x} ${y}`}/>})}</svg><button className="radialOriginNode" onClick={()=>select('video',videos[0])}><i><Play size={17} fill="currentColor"/></i><span><small>传播中心 · 首发</small><b>首发现场视频</b><em>13:46 · 328.6万 VV</em></span></button>{clusterSurfaces.map((surface:any)=><section key={surface.id} className={'entityClusterSurface '+surface.tone} style={{left:`${surface.x/11}%`,top:`${surface.y/5.2}%`,width:`${surface.w/11}%`,height:`${surface.h/5.2}%`}}><header><b>{surface.title}</b><span>{surface.members.length} 个实体</span><small>{surface.meta}</small></header>{surface.members.map((member:any,index:number)=><button key={member.id} className={'clusterEntityDot '+member.kind+(selectedId===member.id?' selected':'')} style={{left:`${dotPositions[index%dotPositions.length][0]}%`,top:`${dotPositions[index%dotPositions.length][1]}%`}} onClick={()=>openSurfaceMember(member)} title={`${member.title} · 点击${member.kind==='视频'?'查看详情':'展开底层证据'}`}><i>{member.kind==='视频'?<Play size={10} fill="currentColor"/>:member.kind==='设备'?<ScanLine size={11}/>:<MessageCircle size={11}/>}</i><span>{member.title}</span></button>)}</section>)}</div>{currentDrill&&<div className="relationClusterExpansion"><header><span><small>簇内展开 · {mergeLabel}</small><b>{currentDrill.title}</b></span><button onClick={()=>setDrilledId(null)}><X size={13}/>收起</button></header><div className="relationExpansionMembers">{currentDrill.nodes.map((node:any)=><button key={node.id} className={node.tone+(selectedId===node.id?' selected':'')} onClick={()=>openNode(node)}><i>{node.kind==='视频'?<Play size={13}/>:node.kind==='账号'?<Users size={13}/>:node.kind==='设备'?<ScanLine size={13}/>:node.kind==='群聊'?<MessageCircle size={13}/>:<Zap size={13}/>}</i><span><small>{node.kind} · {node.role}</small><b>{node.title}</b><em>{node.meta}</em></span>{node.drillKey?<ChevronRight size={13}/>:node.item?<FileText size={12}/>:null}</button>)}</div><footer><span>簇内 {currentDrill.nodes.length} 个实体 · {currentDrill.edges.length} 条证据关系</span><b>展开层展示底层视频、账号与关系证据</b></footer></div>}<div className="relationMapLegend"><span><i/>首发到簇</span><span><i/>簇面内实体</span><small>{currentDrill?'已展开底层证据实体':'不划分传播阶段；簇面表示自动聚类，点表示具体实体'}</small></div></div>
  </div>
