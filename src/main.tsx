...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 return <div className="relationDecisionExplorer">
  ...
  <div className="relationCompactControls"><div className="relationScope"><Network size={14}/><span><small>分析对象</small><b>视频扩散</b></span></div><div className="relationMergeSwitch" role="tablist" aria-label="聚合方式">{([['传播承接','内容相似'],['设备聚合','按设备'],['群聊聚合','按群聊']] as [RelationView,string][]).map(([value,label])=><button key={value} role="tab" aria-selected={!customApplied&&view===value} className={!customApplied&&view===value?'selected':''} onClick={()=>chooseView(value)}>{label}</button>)}<button className={'customMergeButton'+(customApplied?' selected':'')} onClick={()=>setCustomOpen(true)}>＋ 自定义</button></div><div className="relationBasis"><span>成簇依据</span>{(customApplied?ruleSignals:view==='传播承接'?['引用关系','内容相似','搬运二创']:view==='设备聚合'?['共同设备','登录环境','短时切换']:['共同成员','群内分享','跨群转发']).map(item=><i key={item}>{item}</i>)}</div>{currentDrill&&<button className="relationInlineBack" onClick={()=>setDrilledId(null)}>← 返回 {currentDrill.title}</button>}<div className="relationCompactTools"><span>{clusterSurfaces.length} 簇 · {surfaceEntityCount} 实体</span><button aria-label="查找节点"><Search size={14}/></button><button onClick={()=>setZoom(value=>Math.max(.85,Number((value-.05).toFixed(2))))} aria-label="缩小画布"><ZoomOut size={15}/></button><button onClick={()=>setZoom(value=>Math.min(1.15,Number((value+.05).toFixed(2))))} aria-label="放大画布"><ZoomIn size={15}/></button></div></div>
