...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 const rootVisibleNodes=graphNodes[view].filter((node:any)=>node.arrival<=timeIndex)
 const rootVisibleIds=new Set(rootVisibleNodes.map((node:any)=>node.id))
 const rootVisibleEdges=graphEdges[view].filter((edge:any)=>rootVisibleIds.has(edge.from)&&rootVisibleIds.has(edge.to))
 ...
 const selected=(currentDrill?.nodes||[]).find((node:any)=>node.id===selectedId)||displayNodes.find((node:any)=>node.id===selectedId)||displayNodes[0]||graphNodes[view][0]
 ...
 const stageLabels=view==='设备聚合'?['高风险设备簇','多账号设备簇','互动协同设备簇','扩散设备簇']:view==='群聊聚合'?['核心扩散群','风险讨论群','高频分享群','跨群承接群']:['首发视频','关键扩散视频','二创视频','长尾视频']
