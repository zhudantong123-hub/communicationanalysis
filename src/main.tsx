...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 const clusterSummary=customApplied?customSummary:current.decision
 ...
 return <div className="relationDecisionExplorer">
  <div className="relationDecisionHead"><div><b>传播关系链 <SourceBadge type="ai"/></b><span>{current.question}</span></div><div className="relationDecisionSummary"><Sparkles size={14}/><span><small>{customApplied?'自定义聚合规则':'当前 Merge 逻辑'}</small><b>{clusterSummary}</b></span></div></div>
  ...
  <div className="relationViewToolbar"><div className="relationProblemTabs" role="tablist" aria-label="关系分析对象"><button role="tab" aria-selected="true" className="selected">视频扩散</button></div><div className="tools"><button><Search size={14}/>查找节点</button><button onClick={()=>setZoom(value=>Math.max(.85,Number((value-.05).toFixed(2))))} aria-label="缩小画布"><ZoomOut size={15}/></button><button onClick={()=>setZoom(value=>Math.min(1.15,Number((value+.05).toFixed(2))))} aria-label="放大画布"><ZoomIn size={15}/></button></div></div>
  <div className="mergeStrategyBar"><span>Merge 逻辑</span>{([['传播承接','默认关系'],['设备聚合','设备聚合'],['群聊聚合','群聊聚合']] as [RelationView,string][]).map(([value,label])=><button key={value} className={!customApplied&&view===value?'selected':''} onClick={()=>chooseView(value)}>{label}</button>)}<button className={'customMergeButton'+(customApplied?' selected':'')} onClick={()=>setCustomOpen(true)}>＋ 新建聚合规则</button><em>{view==='传播承接'?'主图仅展示视频节点':view==='设备聚合'?'主图仅展示设备簇':view==='群聊聚合'?'主图仅展示群聊簇':''}</em></div>
  <div className="mergeEvidenceTags"><span>当前成簇依据</span>{(customApplied?ruleSignals:view==='传播承接'?['引用关系','内容相似','搬运二创']:view==='设备聚合'?['共同设备','登录环境','短时切换']:['共同成员','群内分享','跨群转发']).map(item=><i key={item}>{item}</i>)}<small>依据由平台提供，用于解释 Merge 结果，不作为节点筛选</small></div>
  <div className="relationBreadcrumb"><button onClick={()=>{setDrilledId(null);chooseView('传播承接')}}>视频扩散</button><ChevronRight size={12}/><b>{customApplied?'自定义规则':mergeLabel}</b>{currentDrill&&<><ChevronRight size={12}/><b>{currentDrill.title}</b><button className="relationBack" onClick={()=>setDrilledId(null)}>返回上一级</button></>}<em>以首发视频为中心 · {clusterSurfaces.length} 个簇面 · {surfaceEntityCount} 个实体节点</em></div>
