...
function InsightCommandCenter({select,activeOpinion,timeFilterIndex,onOpinionSelect,onTimeWindowChange}:{select:(type:string,item:any)=>void,activeOpinion:string|null,timeFilterIndex:number|null,onOpinionSelect:(opinion:string|null)=>void,onTimeWindowChange:(index:number|null)=>void}){
 ...
 return <section className="commandCenter">
  <div className="trendGrid trendOnly">
...
function RiskDiagnosisPanel({value,onChange}:{value:string,onChange:(value:string)=>void}){
 ...
 return <section className="riskDiagnosisPanel">
  <header><div><h2>观点风险诊断 <SourceBadge type="ai"/></h2><p>承接上方趋势异常，结合视频观点、评论反馈和同类场景预期，收敛需要继续定位的风险类型</p></div><span><small>当前判断</small><b>视频—评论反馈背离</b></span></header>
...
function Stats({open,setOpen,select,addToChat,activeOpinion,onOpinionChange,timeContext}){
 ...
 ]}/>}<OpinionRelationMatrix value={opinionRelation} timeContext={timeContext} activeOpinion={activeOpinion} onOpinionSelect={value=>onOpinionChange(activeOpinion===value?null:value)} onChange={value=>{setOpinionRelation(value);setVideoFilter(null);onOpinionChange(null)}}/>{opinionRelation&&<><div className="matrixChannelBridge"><Sparkles size={14}/><span><small>{timeContext.index===null?'观点关系 → 渠道定位':`${timeContext.date} 观点关系 → 渠道定位`}</small><b>{relationProfile.label}主要通过{relationProfile.source==='search'?'站内搜索':relationProfile.source==='share'?'分享回流':'Feed 推荐'}扩散</b></span><em>已按同一时间窗口切换下方渠道并筛选代表视频</em></div><InteractiveSankey select={select} activeVideoFilter={videoFilter} onVideoFilter={setVideoFilter} activeOpinion={activeOpinion} preferredSource={relationProfile.source||timeContext.sourceId}/>
 ...
 <div id="filtered-video-results" className="isFiltered"><Section title="观点异常与代表视频" desc={`已按${timeContext.index===null?'全周期':timeContext.date+' 当日窗口'}“${opinionRelation.replace('|',' × ')}”筛选，当前展示 ${filteredVideos.length} 条代表视频`} action={<button className="videoFilterChip" onClick={()=>{setVideoFilter(null);onOpinionChange(null)}}><ScanLine size={12}/>{relationProfile.label}<X size={12}/></button>} source="data"/><div className="relationResultStrip"><span><small>异常类型</small><b>{relationProfile.label}</b></span><span><small>评论量占比</small><b>{relationMetrics!.share}</b></span><span><small>评论量 / 环比</small><b>{relationMetrics!.volume} · {relationMetrics!.growth}</b></span><span><small>主要渠道</small><b>{relationProfile.source==='search'?'站内搜索':relationProfile.source==='share'?'分享回流':'Feed 推荐'}</b></span></div><div className="videoOpinionFilters"><span>观点筛选</span>{videoOpinionOptions.map(item=><button key={item[0]} className={`${item[2]} ${activeOpinion===item[0]?'selected':''}`} onClick={()=>onOpinionChange(activeOpinion===item[0]?null:item[0])}>{item[0]} <b>{item[1]}</b></button>)}</div><div className="videoAccountFilters"><span>账号类型</span>{['全部','B号','C号'].map(item=><button key={item} className={accountType===item?'selected':''} onClick={()=>setAccountType(item)}>{item}</button>)}</div><div className="cards videoListGrid">{filteredVideos.map((v,i)=><VideoCard key={v.id} v={v} i={i} select={select} addToChat={addToChat}/>)}</div></div></>}</>
}
...
const propagationNetworks=[
 {id:'socialNet',name:'群聊分享传播网络',meta:'86 个核心群 · 214 个外围群',amount:'VV 贡献 31.8%',role:'最高异常 · 分享点击 +214%',category:2,stage:0,anomaly:true,expandable:true,x:510,y:285,size:116,stats:{videos:'1.36万',accounts:'8.7万',actions:'13.3万次',people:'7.1万人',depth:'6层',vv:'2,012万',main:'分享到群 → 点击观看'}},
 {id:'recommendNet',name:'推荐分发网络',meta:'探索池 → 兴趣池 → 主推荐池',amount:'VV 贡献 43.2%',role:'最大规模网络',category:4,stage:0,expandable:true,x:215,y:185,size:104,stats:{videos:'2.84万',accounts:'214.8万',actions:'412.8万次',people:'198.6万人',depth:'4层',vv:'2,734万',main:'推荐曝光 → 点击观看'}},
 {id:'searchNet',name:'搜索回流网络',meta:'热榜词与主动搜索承接',amount:'VV 贡献 12.6%',role:'回流增速 +126%',category:3,stage:0,expandable:true,x:780,y:150,size:84,stats:{videos:'8,426',accounts:'42.6万',actions:'57.3万次',people:'36.2万人',depth:'3层',vv:'797万',main:'搜索 → 结果点击'}},
 {id:'interactionNet',name:'评论互动传播网络',meta:'高赞评论与回复通知导流',amount:'VV 贡献 7.4%',role:'评论导流网络',category:3,stage:0,expandable:true,x:820,y:405,size:70,stats:{videos:'4,218',accounts:'18.4万',actions:'31.2万次',people:'16.8万人',depth:'4层',vv:'468万',main:'评论/回复 → 点击进入'}},
 {id:'creationNet',name:'引用投稿传播网络',meta:'引用、合拍与素材授权投稿',amount:'VV 贡献 5.0%',role:'内容再传播网络',category:1,stage:0,expandable:true,x:245,y:420,size:62,stats:{videos:'5,286',accounts:'4,912',actions:'6,804次',people:'4,126人',depth:'5层',vv:'317万',main:'消费上游 → 引用投稿'}}
]
...
const manipulationException={id:'low-risk-device',name:'集中投稿风险例外',meta:'同设备 50 个账号 · 投稿 126 条 · 暂未形成传播',amount:'传播 VV 仅 238',role:'不因低传播而隐藏',category:0,stage:0,noiseException:true,x:925,y:500,size:38}
...
function SemanticVideoPropagationGraph({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 const allPropagationNetworks=[...propagationNetworks,...secondaryPropagationNetworks]
 const collapsedNetworks=allPropagationNetworks.filter(network=>level===1||network.id!==activeNetwork.id).map(network=>({...network,collapsed:level>1}))
 ...
 const graphNodes:any[]=level===1?[...collapsedNetworks,...(showLowNoise?lowPropagationSamples:[]),manipulationException]:level===2?expandedLocalNodes:expandedEvidenceNodes
 const graphEdges:any[]=level===1?networkTransferEdges:level===2?propagationEdges:evidenceEdges
 ...
 useEffect(()=>{
  ...
  chart.setOption({
   ...
   graphic:[{type:'text',z:30,left:18,top:18,style:{text:level===1?'传播地图 · 点击岛屿原地展开':'聚焦：'+activeNetwork.name,fill:'#aeb7c8',font:'11px sans-serif'}},...focusMasks,...(level>1?[{type:'circle',z:-1,silent:true,shape:{cx:510,cy:300,r:270},style:{fill:'rgba(104,86,166,.07)',stroke:'rgba(163,145,225,.24)',lineWidth:1}},{type:'text',z:30,left:410,top:54,style:{text:level===2?'内部传播结构':'具体证据链',fill:'#c9bdf3',font:'600 12px sans-serif'}}]:[])],
...
function RelationDecisionExplorer({select,timeContext,activeOpinion,onTimeChange}:{select:(type:string,item:any)=>void,timeContext:AnalysisWindow,activeOpinion:string|null,onTimeChange:(value:number)=>void}){
 ...
 return <div className="relationDecisionExplorer">
  <div className="relationDecisionHead"><div><b>传播关系链 <SourceBadge type="ai"/></b><span>{current.question}</span></div></div>
  <ChainTimeline value={timeIndex} onChange={value=>{setTimeIndex(value);setDrilledId(null);setExpandedSurfaceId(null);onTimeChange(value)}} total={surfaceEntityCount+1} label="关系形成时间"/>
